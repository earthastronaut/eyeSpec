...
from core import find_overlap_pts, inv_var_2_var, var_2_inv_var, query_fits_header, eyeSpec_spec
from resampling import get_resampling_matrix
from dependencies import np, deepcopy, scipy, sparse

def combine_orders (obj,line_data=None,include_weights=True,include_varience=True,
...
def co_add_order (ord_A,ord_B,no_interp=False,ai=None,bi=None,wl_bounds=None,linear_interpolate=False,weights=None):
    if no_interp:
        ...
        A_seg, B_seg, B_co_var = spectrum_interp(ord_A,ord_B,ai,bi,wl_bounds,linear_interpolate)
...
++++++++++++++++ get resampling for specific regions ++++++++++++
 ...
 resampling to be the best.
    ...
    if wl_bounds is not None:
        wl_bounds = np.asarray(wl_bounds,dtype=float)
        if wl_bounds.ndim != 1 or wl_bounds.shape[0] != 2: 
    ...
    if linear_interpolate: 
        ...
        norm[(norm == 0)] = 1e-100
        norm_diag = sparse.dia_matrix((1.0/norm,0),shape=(len(norm),len(norm)))
        T = norm_diag*T
        ...
        B_var_seg_diag = sparse.dia_matrix((B_inv_var_seg,0),shape=(len(B_var_seg),len(B_var_seg)))
...
def spectrum_combine (A,B,weights=None,B_co_varience=None):
    ...
    if B_co_varience is not None:
        B_co_var = sparse.csr.csr_matrix(B_co_varience)
