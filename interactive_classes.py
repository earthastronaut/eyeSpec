if __name__ != '__main__':
    from eyeSpec.base_functions import figure_adjust_borders, alt_order_colors
    from eyeSpec.base_classes import Timer
    from eyeSpec.interactive_IO import ProgressSave, SaveOpenChoices, InputOutput
    from eyeSpec.dependencies import os, sys, time, deepcopy, pdb
    from eyeSpec.dependencies import np, scipy, math
    from eyeSpec.dependencies import plt, Figure, FormatStrFormatter, savefig
    from eyeSpec.dependencies import threading, Queue, NewEvent
...
class HistoryAdvanced:
    ...
    def open_history (self,filename):
        if not os.path.exists(filename): raise IOError("File does not exist:"+path)
        ...
        def _checkit_ (timetravel):
            ...
            for i in xrange(len(timetravel)):
                prevstate,func_id,info = timetravel[i]
...
class History:
    ...
    def undo_to (self,i):
        print "!! this will undo everything up to i"
        # max at the furthest back

    def redo_to (self,i): 
        print "!! this will redo everything up to i"
        # max at the current state
    ...
    def display (self):
        ...
        print "======================================="
        print "="*20+format("PAST",'^9')+"="*20
        for i in range(len(self._past)):
            out = [str(i),self._past[i][0]]
            #if self._past[i][0]=='edit point':
            
            print i,self._past[i][0],self._past[i][2]
        ...
        ran.reverse()
        for i in ran:
            out = [str(i),self._future[i][0]]
            #if self._future[i][0]=='edit point':
            
            print i,self._future[i][0],self._future[i][2]
...
class EventConnections:
    """ this bundles some connection abilities for my editor classes """
    ...
    def init_connection_callbacks (self,parent):
        ...
        for con in possible_events:
            ...
            if con_call in dir(parent):
                ...
                self.connections[con] = [None,func]

        # self.connections['key_press_event'] = [None,self.key_press_callback]
...
#        new_keys = key_cfg._short_info.keys()
    ...
    def change_key (self,key,short_info,long_info=None,value=None,add_key_if_missing=True):
        ...
        self.add_key(key,short_info,long_info=None,value=None,insert_mode='replace')
        

    def add_key (self,key,short_info,long_info=None,value=None,insert_mode='append'):
        """
        INPUTS :
    ...
    def display_short_info (self,display=True):
        max_number_characters = max_chr = 80
        out_display = []
        out_display.append(format('key','>10')+'   : Description')
        out_display.append(' '+'-'*79)

        for key in self._display_order:
            out_display.append(self._get_key_display(key,self._short_info,max_chr))
    ...
    def display_long_info (self,key,display=True):
        ...
        output = self._get_key_display(key,self._long_info,max_chr)
...
class DataSelectionBox:
    ...
    def during_mot_notify (self, event, start_pt, data_bounds=None, pressed_dict=None):
        ...
        """
        db = True
        xmin, xmax, ymin, ymax = np.ones(4)
        try: xmin, xmax, ymin, ymax = data_bounds
        except: db = False
    ...
    def during_key_release (self, event, deltax=1.0):
        ...
        xb1, xb2, yb1, yb2 = self.get_bounds()
    ...
    def _check_in_window_visible (self):
        sl = self.get_bounds()
        xmin, xmax, ymin, ymax = self.ax.axis()
        if ((sl[0] > xmax) or (sl[1] < min)) and ((sl[3] < ymin) or (sl[1] > ymax)):        
            return False
        else: return True
    ...
    def selection_box_focus (self, focus, bound=None):
        self.clear_box_variables()
...
class LineSelection:
    def __init__ (self,ax,wl0=0.0,color='y',alpha=0.3,visible=False):
        ...
        self.highlight_line, = ax.plot([wl0,wl0], [ymax,ymin], color=color, alpha=0.65, lw=4, visible=visible, zorder=-50)
...
class PlotLines:
    ...
    def open_lines (self,filename,change_list=True):
        ...
        if change_list: self.change_linelist(line_data,line_info) 
        else: return lines_data,lines_info
          
    def update_info (self,bounds):
...
class PlotDataSingleOrder:
    ...
    def select_order (self,index,direction=None): 
        ...
        if direction == None:
            fnd,i = self._find_non_deleted(index,'+')
            if not fnd: fnd,i = self._find_non_deleted(index,'-')
            if not fnd: raise StandardError("Whoops, looks like all orders were deleted!")
        else:
            fnd,i = self._find_non_deleted(index,direction) 
            if not fnd:
                if direction == '+': fnd,i = self._find_non_deleted(index,'-')
                else: fnd,i = self._find_non_deleted(index,'+') 
            if not fnd: raise StandardError("Whoops, looks like all orders were deleted!")    
        index = i                
    ...
    def set_visible_window (self,bounds):
        return False
...
class eyeSpecBaseLineEditor:
    ...
    def change_linelist (self,line_data,line_info=None):
        self.plotlines.hide()
    ...
    def update (self):
        changed = self.update_text()
        changed = self.plotlines.update()
        return changed
...
class eyeSpecBaseDataPlot:
    ...
    def __init__ (self, ax, spec_obj, auto_scale_focus='full', plotclass=PlotData, **mpl_kwargs):
        ...
        self.plot_data = plotclass(self.ax, self.spec_obj)
        self.overplot_data = []
...
#        
    ...
    def sprog_open (self):
        sprog = self.Input_From_File(event, self._open_choices)
    ...
    def scale_y (self):
        ...
        if ymin == ymax: return
        yran = ymax - ymin
        self.ax.set_ylim(ymin - over_scale * yran, ymax + over_scale * yran)
...
class eyeSpecBaseDataPanel (wx.Panel):
    ...
    def axes_exit_callback (self,event):
...
class eyeSpecBaseEventManager (EventConnections, InputOutput):
    ...
    def get_command_entry (self,event):
        
        print "Use WX python to get an entry"
...
class StandardStars:
    def __init__ (self):
        self.stars = {'sun':OverplotStandardStar("Solar/"),
                      'arcturus':OversplotStandardStar("Arcturus/")}
